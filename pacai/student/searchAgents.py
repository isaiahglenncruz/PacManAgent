...
class CornersProblem(SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        vis_corners = ()
        starting = self.startingPosition

        if starting in self.corners:
            list_start = list(vis_corners)
            list_start.append(starting)
            vis_corners = tuple(list_start)

        self.startState = (starting, vis_corners)
        ...
        for action in Directions.CARDINAL:
            ...
            if not hitsWall:
                ...
                if next in self.corners:
                    if next not in visited_corners:
                        temp = list(visited_corners)
                        temp.append(next)
                        visited_corners = tuple(temp)

                game_node = ((next, visited_corners), action, 1)
                successors.append(game_node)
...
def cornersHeuristic(state, problem):
    ...
    num_to_visit = len(unvisited_corners)
    if num_to_visit == 0:
        return 0

    current_pos = state[0]

    while len(order_to_visit) < num_to_visit:
        curr = 999999999
        curr_dist = None
        for corner in unvisited_corners:
            curr_dist = distance.manhattan(current_pos, corner)
            if curr_dist < curr:
                curr = curr_dist
        for corner in unvisited_corners:
            if curr == distance.manhattan(current_pos, corner):
                order_to_visit.append(corner)
                unvisited_corners.remove(corner)
                current_pos = corner

    current_pos = state[0]
    cost = 0
    for i in order_to_visit:
        cost += distance.manhattan(current_pos, i)
        current_pos = i
    return cost
